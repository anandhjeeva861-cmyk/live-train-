...
export function createAssistant({ getWeatherData, resolveIntent = null, getCatalog = null }) {
  return async function answer({ message, language = 'en-IN', context = {}, history = [] }) {
    ...
    if (plan.intent === 'bookings') return result(say('Use My Bookings for the official IRCTC link. Live Train does not issue valid railway tickets.', 'உங்கள் ரயில் டிக்கெட்டுகளை IRCTC-ல் பார்க்கலாம். My Bookings பகுதியில் அதிகாரப்பூர்வ இணைப்பு உள்ளது.'), { kind: 'navigate', target: 'bookings' });
