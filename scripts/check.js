...
const files = ['server.js', 'assistant.js', ...await scripts('api'), ...await scripts('public'), ...await scripts('scripts'), ...await scripts('backend'), ...await scripts('prisma'), ...await scripts('tests')];
