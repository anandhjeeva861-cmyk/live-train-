...
function knownStation(code) { return stations.find(s => s.code === code); }
function validDate(date) { return /^\d{4}-\d{2}-\d{2}$/.test(date) && !Number.isNaN(Date.parse(date)) && new Date(date).toISOString().slice(0, 10) === date; }
...
export function createAssistant({ getLiveState, getWeatherData, apiKey = '', model = 'gpt-4.1-mini', fetchImpl = fetch }) {
  return async function answer({ message, language = 'en-IN', context = {}, history = [] }) {
    ...
    if (apiKey) {
      try { plan = await modelIntent({ message, language, context, history, apiKey, model, fetchImpl }); mode = 'ai'; }
