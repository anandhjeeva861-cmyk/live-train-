...
- Backend OTP remains a real Resend integration with hashed, expiring, browser-bound, single-use codes, attempt/rate limits, resend and expiry countdowns, idempotent provider requests and session persistence.
...
The user's Resend account/sender is not set up. Configure `RESEND_API_KEY` and `EMAIL_FROM` using [AUTH_SETUP.md](AUTH_SETUP.md) to activate delivery. Tests verify OTP behavior through a test-only provider interception; actual inbox delivery remains unverified. Live railway tracking and ticket issuing require authorized providers and have not been connected.

Validation completed: **41 unit/integration tests and four browser suites passed** (fullstack OTP/catalogue, assistant speech handling, GitHub Pages and Vercel). Checks cover catalogue consistency, intermediate-stop searches, null coordinates and journey days, real photo attribution, route-distance matching, pagination, Google Maps URLs, authentication and legacy booking regressions. Prisma reports no difference between the local database and committed schema. Production dependency audit found zero vulnerabilities. Live inspection loaded actual map tiles and Commons photographs without JavaScript errors.
