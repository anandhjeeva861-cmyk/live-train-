...
## Real authentication follow-up (2026-09-11)

Request: replace demo OTP with Twilio Verify and enable actual Google OAuth while preserving the existing frontend and mobile-first login flow.

Findings: SMS delivery had no provider adapter; both demo flags were enabled; Google credentials were absent. OAuth failures rendered API errors, and existing demo Google identities prevented upgrading the same mobile account.

Implemented: backend/sms.js uses Twilio Verify creation/check APIs with timeouts and sanitized errors. OtpVerification now records provider, provider verification ID and delivery status, with nullable demo-only otpHash. Added migration preserves existing records. Real verification retains browser binding, expiry, attempts, resend limits and one-time consumption. Google callback errors return to the login UI with retry support, valid callback origin checks, state/PKCE and existing token verification. Verified real logins can upgrade a demo identity without changing the user ID or booking ownership. Full real-auth mode rejects old development sessions. Both demo flags default to false, including this workspace's ignored .env. No credentials were fabricated or committed.

Added files: backend/sms.js, scripts/check-auth.js, tests/real-auth.test.js, AUTH_SETUP.md, prisma/migrations/20260910183617_real_otp_provider/migration.sql.
Changed files: backend/auth.js, public/auth.js, prisma/schema.prisma, .env.example, scripts/dev.js, scripts/check-secrets.js, tests/production.test.js, package.json, README.md and this audit. Existing endpoints were integrated; no new endpoint names or database models were needed.
...
Publication verification: commit 03d359e was pushed to origin/main. GitHub Pages deployment and Vercel deployment succeeded. Live browser checks on both public sites returned HTTP 200, rendered the two Chennai/Bengaluru results, showed the browser-demo label, and passed width-390 overflow and page-error checks. Real authentication on these static sites remains unavailable until the documented backend and credentials are configured. GitHub's Linux run passed all 38 backend tests; its first full-stack run exposed a test race at the login retry assertion. The assertion now waits for the asynchronous config response to enable submission rather than checking the pre-existing disabled form immediately.
