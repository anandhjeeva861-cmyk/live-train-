...
export function registerAuth(app) {
  ...
  app.get('/api/auth/config', async (req, res) => {
    ...
    const pending = challenge?.delivered && !challenge.consumed && challenge.attempts < 5 && challenge.expiresAt > new Date() && challenge.sessionHash === digest(req.sessionID);
    res.json({ configured: emailConfigured(), pending: pending ? { email: challenge.email, expiresAt: challenge.expiresAt, retryAt: new Date(challenge.sentAt.getTime() + 60000) } : null });
  });
  ...
  app.post('/api/auth/email/send', limiter(10), async (req, res) => {
    ...
    await writeTransaction(async tx => {
      ...
      const record = { id, email, sessionHash: digest(req.sessionID), codeHash: codeDigest(id, code), expiresAt: new Date(+now + 600000), sentAt: now, attempts: 0, consumed: false, delivered: false };
    ...
    res.json({ sent: true, email, expiresAt: new Date(+now + 600000), retryAt: new Date(+now + 60000) });
  });
  app.post('/api/auth/email/verify', limiter(30), async (req, res) => {
    ...
    const result = await writeTransaction(async tx => {
      ...
      if (!row || row.email !== email || row.sessionHash !== digest(req.sessionID) || row.consumed || !row.delivered || row.expiresAt <= new Date() || row.attempts >= 5) return null;
      ...
      await tx.emailVerification.update({ where: { id: row.id }, data: { consumed: true } });
      ...
      if (!user && guest?.mobileNumber.startsWith('guest:') && !guest.email) {
        user = await tx.user.update({ where: { id: guest.id }, data: { email, emailVerified: true } });
      } else {
        user = await tx.user.upsert({ where: { email }, create: { email, emailVerified: true, name: email.split('@')[0], mobileNumber: 'email:' + crypto.randomUUID() }, update: { emailVerified: true } });
