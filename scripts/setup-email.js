...
import { z } from 'zod';
import { smtpTransport } from '../backend/email.js';
...
export function gmailEnvironment(input) {
  ...
  return { EMAIL_PROVIDER: 'smtp', SMTP_HOST: 'smtp.gmail.com', SMTP_PORT: '465', SMTP_USER: email, SMTP_PASS: password, EMAIL_FROM: `Live Train <${email}>` };
}

function setupFailure(error) {
  if (error.code === 'EAUTH') return 'Gmail rejected the sender login. Use a valid Gmail App Password for this same Gmail address, with Google 2-Step Verification enabled. Your normal Google password will not work.';
  if (['ETIMEDOUT', 'ECONNECTION', 'ESOCKET', 'EDNS'].includes(error.code)) return 'Could not connect to Gmail SMTP (smtp.gmail.com:465). Check your internet connection and whether a firewall or VPN is blocking SMTP, then retry.';
  if (error.code === 'ETLS') return 'Could not establish a secure connection to Gmail. Check your computer clock and network settings, then retry.';
  return 'Could not verify the Gmail sender. Check the App Password and your connection, then retry. No settings were saved.';
}
...
function page(token, loginUrl, message = '', saved = false) {
  return `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Live Train — Email setup</title>
  ...
  <main><small>LIVE TRAIN · LOCAL EMAIL SETUP</small><h1>${saved ? 'Gmail sender connected' : 'Connect Gmail for OTP'}</h1>
  ${message ? `<p class="message" role="${saved ? 'status' : 'alert'}">${escape(message)}</p>` : ''}
  ${saved ? `<p>Gmail accepted your sender login. Your credentials were saved only in this project's private <code>.env</code>. Open Live Train, click Profile, fill in your details, and send an email OTP. Enter the code from your inbox to confirm delivery; check Spam too.</p><a class="button" href="${escape(loginUrl)}">Open Live Train Profile</a>` : `<p>உங்கள் Gmail மூலம் verification code அனுப்ப இந்த setup-ஐ முடிக்கவும்.</p><ol><li>Turn on <a href="https://myaccount.google.com/signinoptions/two-step-verification" target="_blank" rel="noopener noreferrer">Google 2-Step Verification</a>.</li><li>Open <a href="https://myaccount.google.com/apppasswords" target="_blank" rel="noopener noreferrer">Google App Passwords</a>, create one named Live Train, and enter it below. <a href="https://support.google.com/accounts/answer/185833" target="_blank" rel="noopener noreferrer">Google setup help</a></li></ol>
  <form method="post" action="/configure" autocomplete="off"><input type="hidden" name="token" value="${token}"><label>Sender Gmail address<input type="email" name="email" autocomplete="off" maxlength="254" required></label><label>Google App Password<input type="password" name="password" autocomplete="off" maxlength="100" required aria-describedby="password-help"></label><small id="password-help">Use the 16-character App Password. Keep it out of chat and screenshots.</small><button type="submit">Verify sender &amp; save</button></form><p>Credentials are sent to Google over TLS to check the sender login, then saved in this project's private <code>.env</code>. This page is available only on this computer. No email is sent by this setup check.</p>`}
  <hr><h2>For your Vercel website</h2><p>This connects the local app only. The public website also needs the Node backend on persistent hosting. Set these same email settings privately on that backend, then connect its HTTPS origin in Vercel and redeploy as described in <code>DEPLOYMENT.md</code>. Never put the App Password in public/config.js.</p></main></html>`;
}

export function createEmailSetup({ envFile = defaultEnvFile, transportFactory = smtpTransport, loginUrl = 'http://localhost:4174/profile', nodeEnv = process.env.NODE_ENV } = {}) {
  ...
  let busy = false, lastAttempt = 0, saved = false;
  app.disable('x-powered-by');
  app.use((req, res, next) => {
    const host = `127.0.0.1:${req.socket.localPort}`;
    // no-referrer makes native browser POST forms send Origin: null. Preserve
    // the same-origin form's Origin while withholding referrers from other sites.
    res.set({ 'Cache-Control': 'no-store', 'Referrer-Policy': 'same-origin', 'X-Frame-Options': 'DENY', 'X-Content-Type-Options': 'nosniff', 'Content-Security-Policy': "default-src 'none'; style-src 'unsafe-inline'; form-action 'self'; frame-ancestors 'none'; base-uri 'none'" });
    ...
    if (!['GET', 'HEAD'].includes(req.method) && req.get('origin') !== `http://${host}`) return res.status(403).type('html').send(`<p>This submission did not come from the local setup page. <a href="http://${host}/">Reload Live Train email setup</a> and use its form. Settings were not changed.</p>`);
    next();
  });
  app.use(express.urlencoded({ extended: false, limit: '4kb' }));
  app.get('/', (_req, res) => res.type('html').send(page(token, loginUrl)));
  app.get('/configure', (_req, res) => res.redirect(303, '/'));
  app.get('/saved', (_req, res) => saved
    ? res.type('html').send(page(token, loginUrl, 'Gmail verified. Saved locally in the private .env file.', true))
    : res.redirect(303, '/'));
  app.post('/configure', async (req, res) => {
    ...
    busy = true; lastAttempt = Date.now(); saved = false;
    let transport, verified = false;
    try {
      transport = transportFactory(settings);
      verified = await transport.verify() === true;
      if (!verified) throw new Error('SMTP verification was not confirmed');
      await saveEmailEnvironment(envFile, settings);
      saved = true;
      // Refresh the success page without replaying the password-bearing POST.
      res.redirect(303, '/saved');
    } catch (error) {
      const message = verified ? 'Gmail accepted the sender login, but the private .env file could not be saved. Check file permissions and available disk space, then retry.' : setupFailure(error);
      res.status(verified ? 500 : 502).type('html').send(page(token, loginUrl, message));
    } finally { try { transport?.close(); } finally { busy = false; } }
  });
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const app = createEmailSetup({ nodeEnv: process.env.NODE_ENV || local.NODE_ENV, loginUrl: `http://localhost:${appPort}/profile` });
  const server = app.listen(port, '127.0.0.1', () => console.log(`Live Train email setup: http://127.0.0.1:${port}/\nEnter the App Password only in that local page. Press Ctrl+C to close setup.`));
